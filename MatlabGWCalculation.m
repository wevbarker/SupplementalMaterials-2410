...
% List of dataset variants
variants = {'1e6g', '1e7g', '1e8g', '1e9g'};
...
cg = 0.4;
OmegaR0 = 2.47e-5;
% Loop over all variants and suffixes
for i = 1:length(variants)
    for j = 1:length(suffixes)
        % Build the filename for the power spectrum and model data
        variant_name = [variants{i}, suffixes{j}];
        power_spectrum_file = fullfile(script_folder, [variant_name, '_PowerSpectrum.txt']);
