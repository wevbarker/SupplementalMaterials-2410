...
Once the model scripts have been run, the stochastic GW spectra can be computed. The integration is performed using Mathematica, to avoid errors encountered when using Python: we posit that these errors would not be difficult to resolve with further development, allowing for a fully open-source pipeline. The Mathematica notebook `PBH_GW.nb` records the outcome of this integration procedure, as produced by the one-line script: 
```mathematica
...
```
Within `PBH_GW.m`, the global parameters `$Compute=True` and `$TheProcessorCount=100` reflects the intention to actually compute the spectra in an HPC environment (for which the command line is more appropriate). By setting `$Compute=False`, the notebook will instead load the pre-computed spectra from binaries such as `1e6g_lower_GW.mx` and plot them (evidently, the notebook is more suitable in this case).
